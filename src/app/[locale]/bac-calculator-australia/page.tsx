...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const localizedUrl = `${SITE_URL}${localePrefix}/bac-calculator-australia`;
  const imageUrl = `${SITE_URL}/og-image.svg`;

  return {
    title: `${t('au.title')} | ${metadata('siteName')}`,
    description: t('au.description'),
    alternates: {
      canonical: localizedUrl,
      languages: getLanguageAlternates('/bac-calculator-australia'),
    },
    openGraph: {
      type: 'article',
      locale,
...
export default async function BacCalculatorAustraliaPage({ params }: PageProps) {
  const locale = (locales.includes(params.locale as Locale) ? params.locale : defaultLocale) as Locale;
  const t = await getTranslations({ locale, namespace: 'countryBac' });

  const docsMarkdown = `
## ${t('au.legalSectionTitle')}

${t('au.legalSectionBody')}

## ${t('au.safetySectionTitle')}

${t('au.safetySectionBody')}
`;

  const faqItems = [
    {
      question: t('au.faq.q1'),
      answer: t('au.faq.a1'),
    },
    {
      question: t('au.faq.q2'),
      answer: t('au.faq.a2'),
    },
    {
      question: t('au.faq.q3'),
      answer: t('au.faq.a3'),
    },
  ];
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-5xl px-4 py-8 md:py-12">
          ...
          <div id="tool" className="mb-16">
            <BacCalculator defaultCountryCode="AU" enableLocalStorage />
          </div>
