...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'accuracyShort' });
  const localePrefix = locale === defaultLocale ? '' : `/${locale}`;
  const localizedUrl = `${SITE_URL}${localePrefix}/are-bac-calculators-accurate`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: `${SITE_URL}/are-bac-calculators-accurate`,
        'x-default': `${SITE_URL}/are-bac-calculators-accurate`,
      },
    },
    openGraph: {
      type: 'article',
...
export default async function AreBacCalculatorsAccuratePage({ params }: PageProps) {
  const locale = (locales.includes(params.locale as Locale) ? params.locale : defaultLocale) as Locale;
  const t = await getTranslations({ locale, namespace: 'accuracyShort' });
...
`;
  ...
  const articleSchema = {
    ...
    headline: t('title'),
    description: t('description'),
    mainEntityOfPage: `${SITE_URL}/are-bac-calculators-accurate`,
  };
